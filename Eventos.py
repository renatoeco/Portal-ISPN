...
CALENDAR_ID = "a6064be275890455174e0fa13aff800b717574af08de64240cf751570b7b771c@group.calendar.google.com"

#"id":"a6064be275890455174e0fa13aff800b717574af08de64240cf751570b7b771c@group.calendar.google.com"
#"id":"769cd059e9dab43da0d8b9e58faa323769720f6735ca73dceb8347d9e6b0c303@group.calendar.google.com"

components.html(f"""
<div style="display:flex; justify-content:center;">
    <iframe 
        src="https://calendar.google.com/calendar/embed?src={CALENDAR_ID.replace('@','%40')}&ctz=America%2FSao_Paulo"
        style="border:0; width:100%; max-width:1200px; height:700px;"
        frameborder="0"
        scrolling="no">
    </iframe>
</div>
...
# ##################################################################
# CONEXÃO COM GOOGLE SHEETS E GOOGLE CALENDAR
# ##################################################################
...
def get_calendar_service():
    ...
    creds_dict = st.secrets["credentials_drive_teste"]
...
def sincronizar_evento_google(row):

    codigo = str(row["Código do evento:"]).strip()

    if row["Status"] == "Cancelado":
        return None

    data_str = row.get("Data do evento")
    ...
    evento = {
        "summary": f"{row['Atividade:']} ({codigo})",
        "start": {"date": inicio.isoformat()},
        "end": {"date": (fim + pd.Timedelta(days=1)).isoformat()},
        "colorId": CORES_GOOGLE.get(row["Status"], "1"),
        "description": f"Código do evento: {codigo}"
    }

    evento_mongo = eventos.find_one({"codigo": codigo})

    try:

        if evento_mongo and evento_mongo.get("google_event_id"):

            calendar_service.events().update(
                calendarId=CALENDAR_ID,
                eventId=evento_mongo["google_event_id"],
                body=evento
            ).execute()

            return evento_mongo["google_event_id"]

        else:

            criado = calendar_service.events().insert(
                calendarId=CALENDAR_ID,
                body=evento
            ).execute()
        ...
        st.error(f"Erro ao sincronizar evento {codigo}: {e}")
        return None


def sincronizar_com_google(df_eventos):

    for _, row in df_eventos.iterrows():
        sincronizar_evento_google(row)
...
def sincronizar_eventos_novos(df_eventos):
    ...
    for codigo in codigos_novos:
        ...
        criado = sincronizar_evento_google(row)
...
def extrair_datas(texto):
    ...
    padrao = r'Data de Início:\s*(\d{2}-\d{2}-\d{4}).*?Data de Fim:\s*(\d{2}-\d{2}-\d{4})'
...
sincronizar_eventos_novos(df_eventos)
#st.cache_data.clear()
